...
def validate_roi(roi: float) -> float:
    ...
    if roi < 0:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="ROI must be non-negative"
        )
...
def validate_volume(volume: int) -> int:
    ...
    if volume < 0:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Volume must be non-negative"
        )
    if volume > 2_147_483_647:  # Max 32-bit signed integer
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Volume exceeds maximum allowed value"
        )
    return volume
...
def validate_slot(slot: str) -> str:
    ...
    """
    valid_slots = [
        "head", "cape", "neck", "ammo", "weapon", "shield", "body",
        "legs", "gloves", "boots", "ring", "two_handed"
    ]
...
def ROIQuery(default: float = 0.0, **kwargs):
    ...
    return Query(
        ...
        ge=0.0,
        le=10000.0,
        description="Minimum ROI percentage (0 to 10000)",
        **kwargs
    )
...
def VolumeQuery(default: int = 0, **kwargs):
    ...
    return Query(
        ...
        ge=0,
        le=2_147_483_647,
        description="Minimum volume (0 to 2,147,483,647)",
        **kwargs
    )
