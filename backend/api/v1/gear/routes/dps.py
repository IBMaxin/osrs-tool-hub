...
from backend.services.gear.dps import compare_dps, DPSComparisonLoadout
...
async def calculate_dps(
    request: DPSRequest, session: Session = Depends(get_session)
) -> dict[str, object]:
    """
...
async def compare_dps_endpoint(
    ...
    loadout_objects: list[DPSComparisonLoadout] = []
    has_weapon = False
    for loadout_input in request.loadouts:
        ...
        for slot, item_id in loadout_input.loadout.items():
            if item_id is not None:
                ...
                items[slot] = None
        has_weapon = has_weapon or bool(items.get("weapon"))
