...
async def sync_all():
    ...
    with Session(engine) as session:
        ...
        if items_without_stats_count > 0:
            ...
            # Verify
            items_with_stats_after = len(list(session.exec(select(Item).where(Item.slot.isnot(None))).all()))
        ...
        final_items_with_stats = len(list(session.exec(select(Item).where(Item.slot.isnot(None))).all()))
