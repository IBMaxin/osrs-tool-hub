...
class TestGearBestLoadoutEndpoint(BaseE2ETest):
    ...
    def test_get_best_loadout(self, client: TestClient, session: Session, sample_items: list[Item]):
...
class TestGearUpgradePathEndpoint(BaseE2ETest):
    ...
    def test_get_upgrade_path(self, client: TestClient, session: Session, sample_items: list[Item]):
...
class TestGearDPSEndpoint(BaseE2ETest):
    ...
    def test_calculate_dps(self, client: TestClient, session: Session, sample_items: list[Item]):
        """Test calculating DPS for a loadout."""
        payload = {
            "loadout": {"weapon": 4151},  # Abyssal whip
            "combat_style": "melee",
            "attack_type": "slash",
            "player_stats": {"attack": 70, "strength": 70},
        }
