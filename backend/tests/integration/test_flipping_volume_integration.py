    ...
    async def test_volume_sync_and_flipping_integration(self):
        ...
        with next(get_session()) as session:
            ...
            for opp in opportunities:
                ...
                expected_roi = (
                    (opp["margin"] / opp["sell_price"]) * 100 if opp["sell_price"] > 0 else 0
                )
                assert (
                    abs(opp["roi"] - expected_roi) < 0.01
                ), f"ROI calculation incorrect: {opp['roi']} vs {expected_roi}"
    ...
    async def test_flipping_service_with_24h_volume_ranking(self):
        ...
        with next(get_session()) as session:
            ...
            if len(margin_x_volumes) > 1:
                ...
                assert (
                    margin_x_volumes == sorted_volumes
                ), "Opportunities should be sorted by margin_x_volume descending"
    ...
    async def test_flip_opportunity_model_with_volume_fields(self):
        ...
        with next(get_session()) as session:
            ...
            opportunities = flipping_service.find_best_flips(
                budget=5_000_000, min_roi=1.0, min_volume=0, exclude_members=False
            )
