...
def mock_wiki_client():
    ...
    # Mock mapping data
    mock_client.fetch_mapping = AsyncMock(return_value=[
        {
            "id": 4151,
            "name": "Abyssal whip",
            "members": True,
    ...
    # Mock price data
    mock_client.fetch_latest_prices = AsyncMock(return_value={
        "data": {
            "4151": {
                "high": 1500000,
                "low": 1400000,
...
async def test_seed_prices_success(test_engine, mock_wiki_client):
    ...
    with patch('backend.scripts.seed_prices.engine', test_engine), \
