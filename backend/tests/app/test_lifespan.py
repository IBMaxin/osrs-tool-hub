...
async def test_create_db_and_tables(test_engine):
    ...
    with patch('backend.app.lifespan.engine', test_engine), \
...
async def test_lifespan_seed_failure_no_existing_data(mock_app, test_engine):
    ...
    with patch('backend.app.lifespan.engine', test_engine), \
        ...
        async with lifespan(mock_app):
            ...
            warning_calls = [call for call in mock_logger.warning.call_args_list if "No slayer data" in str(call)]
...
async def test_lifespan_seed_failure_with_existing_data(mock_app, test_engine):
    ...
    with patch('backend.app.lifespan.engine', test_engine), \
        ...
        async with lifespan(mock_app):
            ...
            info_calls = [call for call in mock_logger.info.call_args_list if "existing slayer data" in str(call).lower()]
